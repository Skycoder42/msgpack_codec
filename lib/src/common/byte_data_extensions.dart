...
extension ByteDataExtensions on ByteData {
  ...
  void setBigUint64(
    ...
    final signedValue = value.toSigned(value.bitLength);
    if (signedValue.isValidInt) {
      setUint64(byteOffset, signedValue.toInt(), endian);
      return;
    }
  ...
  BigInt getBigUint64(int byteOffset, [Endian endian = Endian.big]) =>
      BigInt.from(getUint64(byteOffset, endian)).toUnsigned(64);

  @pragma('vm:prefer-inline')
  BigInt getBigInt64(int byteOffset, [Endian endian = Endian.big]) =>
      BigInt.from(getInt64(byteOffset, endian)).toSigned(64);
}
